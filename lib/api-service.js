...
export async function handleContactRequestsRequest(method, body, request = null) {
  ...
  if (method === 'PUT') {
    ...
    if (index === -1) {
      return { statusCode: 404, payload: { error: 'Contact request not found' } };
    }

    const payloadData = requestBody.updatedData || requestBody;
    if (payloadData.status) {
      requests[index].status = String(payloadData.status).trim();
    }
...
export async function handleUsersRequest(method, body, request = null) {
  requireAdminSession(request);
  const requestBody = parseBody(body);
  ...
  if (method === 'POST') {
    const input = normalizeUserInput(requestBody.user || requestBody);
    const password = String((requestBody.user || requestBody).password || '');
  ...
  if (method === 'DELETE') {
    ...
    if (users.length <= 1) {
      return {
        ...
        payload: {
          error: 'LAST_USER',
          message: 'Nao e possivel remover o unico usuario administrador.',
        },
      };
    }
    ...
    return {
      statusCode: 200,
      payload: { users: filteredUsers.map(sanitizeUser), storage: getStorageModeLabel() },
    };
  }
